...
def _raw(block, index):
    return {"transactionHash":"0xabc","blockNumber":hex(block),"logIndex":hex(index),
            "topics":["0x01","0x02"],"removed":False}

def _http_429():
    return urllib.error.HTTPError("https://example.invalid",429,"Too Many Requests",{},None)
...
def test_rpc_resilience_v2_preserves_log_semantics(monkeypatch):
    ...
    logs,_=r.adaptive_get_logs(FakeRpc(responder),["0x01"],10,15,initial_window=2)
    assert [(x["block_number"],x["log_index"]) for x in logs]==[(10,1),(12,1),(15,1)]
...
def test_persistent_429_splits_window_then_recovers(monkeypatch):
    ...
    def responder(method,params):
        ...
        if end-start+1>10:
            ...
            raise _http_429()
        return []
    _,windows=r.adaptive_get_logs(FakeRpc(responder),["0x01"],1,20,initial_window=20)
...
def test_checkpoint_resume_and_digest_validation(monkeypatch,tmp_path):
    ...
    def responder(method,params):
        ...
        if start>=11 and state["fail"]: raise RuntimeError("provider down")
        return []
    rpc=FakeRpc(responder)
    with pytest.raises(RuntimeError): r.adaptive_get_logs(rpc,["0x01"],1,20,initial_window=10)
